...
const paginatedDataCustomers = computed(() => {
  ...
  totalItems.value = filteredPIC.value.length;
  return filteredPIC.value.slice(start, end);
});
...
const downloadCSV = () => {
  ...
  link.setAttribute("download", "view_data_pic.csv");
      ...
      >
        ...
        <tbody>
          ...
          >
            ...
            <td class="px-4 py-3">{{ c.deal?.handler }}</td>
            <td class="px-4 py-3">{{ c.deal?.link_drive }}</td>
            <td class="px-4 py-3">{{ c.deal?.total_pax }}</td>
            <td class="px-4 py-3">{{ c.deal?.activity }}</td>
            <td class="px-4 py-3">{{ c.deal?.payment_status }}</td>
